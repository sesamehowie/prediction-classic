...
contract PredictionClassic is Ownable, ReentrancyGuard, Pausable {
    ...
    event HouseEdgeUpdated(uint8 houseEdge);
    ...
    function _calculateRewards(uint256 roundId) internal {
        ...
        if (
            ...
            (round.pumpAmount == 0 || round.dumpAmount == 0)
        ) {
            round.roundCancelled = true;
            round.rewardsCalculated = true;
            _refundPlayersInRound(roundId);
        ...
        round.winner = round.closePrice > round.lockPrice
            ? Position.Pump
            : Position.Dump;
        uint256 rewardDividend = round.betVolume -
            (round.betVolume * houseEdge) /
            100;
        ...
        for (uint256 i = 0; i < users.length; ) {
            ...
            if (!bet.claimed) {
                uint256 winAmount = round.winner == Position.Pump
                    ? (bet.pumpAmount * rewardDividend) / round.pumpAmount
                    : (bet.dumpAmount * rewardDividend) / round.dumpAmount;
                if (winAmount > 0) {
                    userClaimableAmount[user] += winAmount;
                    userClaimableRounds[user][roundId] = winAmount;
                    round.totalPayout += winAmount;
                }
            }
            unchecked {
                i++;
    ...
    function calculatePotentialPayout(
        ...
        Round storage round = rounds[roundId];
        uint256 pumpAmt = round.pumpAmount;
        uint256 dumpAmt = round.dumpAmount;
        uint256 betVolume = round.betVolume;

        if (pumpAmt == 0 || dumpAmt == 0) {
            return betAmount;
        } else {
            uint256 payoutDividend = betVolume -
                ((betVolume * houseEdge) / 100);
            uint256 potentialReward = position == Position.Pump
                ? (betAmount * payoutDividend) / pumpAmt
                : (betAmount * payoutDividend) / dumpAmt;
    ...
    function setHouseEdge(uint8 _houseEdge) external onlyOwner {
        require(
            _houseEdge > 0 && _houseEdge != houseEdge,
            "House edge should be greater than 0 and/or not equal to the current house edge."
        );
